 @if ($paginator->lastPage() > 1)
    <ul class="pagination">
        ...
        @if ($paginator->currentPage() != 1 && $paginator->lastPage() >= 10)
            <li> 
                <a href="{{ URL::to($Purl.'1')}}" >
                    <<
                </a>
            </li>
        @endif
        ...
        @if($paginator->currentPage() != 1)
            ...
            <li>
                <a href="{{URL::to($Purl.($paginator->currentPage()-10) )}}" >
                    <
                </a>
            </li>
        @endif
        ...
        @for($i = max($paginator->currentPage()-0, 1); $i <= min(max($paginator->currentPage()-0, 1)+9,$paginator->lastPage()); $i++)
                <li class="{{ ($paginator->currentPage() == $i) ? ' active' : '' }}">
                    <a href="{{URL::to($Purl.$i)}}">{{ $i }}</a>
                </li>
        @endfor
        ...
        @if ($paginator->currentPage() != $paginator->lastPage() && $paginator->lastPage() >= 10)
            <li>
                <a href="{{URL::to($Purl.$paginator->lastPage())}}   " >
                    >>
                </a>
            </li>
        @endif
