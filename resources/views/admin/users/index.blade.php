...
<!-- /search form -->
	...
	<tbody>
		...
		<tr>
		 ...
		 <td> 
		@if($Users->Status!=1) 
		<a href="{{URL::to('admin/users/'.base64_encode($Users->id).'/active')}}" class="btn btn-danger btn-sm btn-icon icon-left" type="button">
						Inactive   
						<i class="entypo-cancel"></i>
					</a>
	  @else
		 <a href="{{URL::to('admin/users/'.base64_encode($Users->id).'/inactive')}}" class="btn btn-green btn-sm btn-icon icon-left" type="button">
						Active
		 ...
		 <td>
				<a  href="{{URL::to('admin/users/'.base64_encode($Users->id).'/edit')}}"   class="btn btn-default btn-sm btn-icon icon-left">
					<i class="entypo-pencil"></i>
					Edit
				</a>
		 
				<a href="#"  data-href="{{URL::to('admin/users/'.base64_encode($Users->id).'/delete')}}" class="btn btn-danger btn-sm btn-icon icon-left"  data-toggle="modal" data-target="#confirmDelete" data-title="Delete User" data-message="Are you sure you want to delete this User?" >
					<i class="entypo-cancel"></i>
					Delete
				</a>
				  
				<a href="{{URL::to('admin/users/'.base64_encode($Users->id).'/detail')}}" class="btn btn-info btn-sm btn-icon icon-left">
...
<div class="dataTables_paginate paging_bootstrap">
     <?php $Purl='admin/users/index?page='; ?>
...
$(document).ready(function(){
  ...
  $('.inactiveall').click(function(){
    ...
    $.ajax({
       url:'{{url()}}/admin/users/inactiveall', 
      type: "post",
	  data: {_token: CSRF_TOKEN,checkboxvalue:checkboxvalue},
    //data: {'_token': $('input[name=_csrf-token]').val()},
      success: function(data){
