...
<div class="col-sm-12">
    ...
    <div class="panel panel-primary  " data-collapsed="0">		 		
        <div class="panel-body">				
            <form autocomplete="off" action="{{ URL::to('admin/staticpages/index') }}" method="post" class="form-horizontal">
    ...
    <tbody>
        ...
        <tr>
          ...
          <td>{{ $Users->title}}</td>
            <td>
                 <?php
                                            if (strlen($Users->description) > 30) {
                                                echo substr($Users->description, 0, 15) . '...';
                                            } else {
                                                echo substr($Users->description, 0, 15);
                                            }
                                            ?>
            ...
            <td> 
                @if($Users->Status!=1) 
                <a href="{{URL::to('admin/staticpages/'.base64_encode($Users->id).'/active')}}" class="btn btn-danger btn-sm btn-icon icon-left" type="button">
                    Inactive   
                    <i class="entypo-cancel"></i>
                </a>
                @else
                <a href="{{URL::to('admin/staticpages/'.base64_encode($Users->id).'/inactive')}}" class="btn btn-green btn-sm btn-icon icon-left" type="button">
                    Active
            ...
            <td>
                <a  href="{{URL::to('admin/staticpages/'.base64_encode($Users->id).'/edit')}}"   class="btn btn-default btn-sm btn-icon icon-left">
                ...
                <a href="{{URL::to('admin/staticpages/'.base64_encode($Users->id).'/detail')}}" class="btn btn-info btn-sm btn-icon icon-left">
...
<!--        <div class="dataTables_info" id="table-2_info">
    ...
    <div class="col-xs-6 col-right">
        <div class="dataTables_paginate paging_bootstrap">
            <?php $Purl='admin/staticpages/index?page='; ?>
    ...
    $(document).ready(function () {
        $('.activeall').click(function () {
            ...
            if ($(".case:checked").length > 0) {
                ...
                $.ajax({
                    url: '{{url()}}/admin/staticpages/activeall',
                    type: "post",
        ...
        $('.inactiveall').click(function () {
            ...
            if ($(".case:checked").length > 0) {
                ...
                $.ajax({
                    url: '{{url()}}/admin/staticpages/inactiveall',
                    type: "post",
                    data: {_token: CSRF_TOKEN, checkboxvalue: checkboxvalue},
                    //data: {'_token': $('input[name=_csrf-token]').val()},
                    success: function (data) {
        ...
        $('.deleteall').click(function () {
            if ($(".case:checked").length > 0) {
                ...
                $.ajax({
                    url: '{{url()}}/admin/staticpages/deleteall',
                    type: "post",
                    data: {_token: CSRF_TOKEN, checkboxvalue: checkboxvalue},
                    success: function (data) {
                        window.location = self.location;
